...
def get_post(post_url, is_pinned=False):
    ...
    post_info = soup.select("ul.board-etc-wrap > li") 
    department = post_info[0].get_text(strip=True) if len(post_info) >= 1 else "" 
    views_li = post_info[1] if len(post_info) >= 2 else "" 
    if views_li:
        span = views_li.select_one("span.board-mg-l10")
        views = int(span.get_text(strip=True)) if span else 0
    else:
        views = 0
    date = post_info[2].get_text(strip=True) if len(post_info) >= 3 else "" 
    ...
    post = {
        ...
        "post_link": post_url.replace(f"&article.offset=0&articleLimit={ARTICLE_NUM}", ""),
        "views": views,
        "date": date,
        "files": files,
        "images": images,
...
def get_skku_post(post_url):
    ...
    date = title_div.select_one("span.date").get_text(strip=True) 
...
if __name__ == "__main__":
    ...
    if check_outdated("skku_main_posts.json") or args.force_update:
        ...
        temp_file = "skku_main_posts.json.tmp"
